...
SNMPRequest::SNMPRequest(int pduType, std::string oid, int count, SNMPRemoteAgent * pRemoteAgent) {
	_pduType = pduType;
	_state = SNMP_RAREQ_STATE_NONE;
	_error = SNMP_ERROR_NOERROR;
	_requestId = 0;
	...
	_pRemoteAgent = pRemoteAgent;
	m_pResult = new SNMPObject();
	m_pRequest = new SNMPObject(oid);
	m_bIsRecv = false;
//	_request.insert(_request.end(), new SNMPObject(oid));
}
...
void SNMPRequest::addToResult(SNMPObject & object) {
	...
	if (m_pResult) {
		m_pResult->_syntax = object._syntax;
				
		//> 1.1.2.9
		//> 문자열
		if (object._binary) {
			m_pResult->setOctets(object._octets, object._octetsLen);
		} else {
			m_pResult->_string = object._string;
		}
	//	m_pResult->_binary = false;
		m_pResult->_binary = object._binary;

		//> 180305 정수형 세분화
		//> 정수		
		m_pResult->_uint = object._uint;		 
		m_pResult->_int = object._int;
	}
}
...
void SNMPRequest::wait(DWORD timeout) {
	...
	if (m_bIsRecv == false) {
	//	_dw("wait...\n");
		waitRet = ::WaitForSingleObject(_hWait, timeout);	
